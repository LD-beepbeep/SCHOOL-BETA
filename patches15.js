/* ================================================================
   StudentOS — patches15.js  v4  (surgical)

   ROOT CAUSE FIXES:
   1. Widget modal emojis  → MutationObserver wins over ALL openModal
      patches regardless of load order
   2. Avatar too small     → patch _p10syncAvatar() directly with
      element.style.setProperty('width','96px','important')
   3. Forum name           → FULLY REPLACE forumSubmitPost,
      forumSubmitReply, window._srSubmit using our own addDoc import
   4. Task drag & drop     → always-visible handles, robust patch
   5. Re-renders           → specific fixes

   ADD LAST in index.html:
     <link rel="stylesheet" href="patches14.css">
     <script type="module" src="patches14.js"></script>
   ================================================================ */
...
/* local date — no UTC shift (fixes Belgium UTC+1 showing wrong day) */
...
function _fixSearch() {
    ...
    btn.onclick = () => typeof window._p11openSearch==='function'
        ? window._p11openSearch()
...
/* ================================================================
   3. AVATAR — patch _p10syncAvatar to force large size via JS
      (beats CSS, beats inline styles, always wins)
   ================================================================ */
function _fixAvatarSize(el) {
    ...
    if (!el.querySelector('img'))
        el.style.setProperty('font-size','2.8rem','important');
}
...
function _patchAvatarSync() {
    function _try() {
        ...
        window._p10syncAvatar = function() {
            _orig();
            _fixAvatarSize(document.getElementById('avatar-preview'));
            _fixAvatarSize(document.getElementById('p10-avatar-preview-tab'));
        };
    }
    _try();

    // Also patch renderProfileDisplay to fix size after every call
    function _tryRPD() {
...
/* ================================================================
   4. FORUM DISPLAY NAME — fully replace submit functions
      using our own Firebase imports so the closure var
      _displayName in forum.js is never used.
   ================================================================ */
function _getDisplayName() {
    return _dbG('os_name','').trim()
        || document.querySelector('#profile-name-input')?.value?.trim()
        || 'Student';
}
...
function _replaceForumSubmitPost() {
    function _try() {
        ...
        window.forumSubmitPost = async function() {
            ...
            const tEl=document.getElementById('forum-new-title');
            const bEl=document.getElementById('forum-new-body');
            ...
            try {
                await addDoc(collection(_db,'forum_posts'),{
                    uid:_uid, displayName:_getDisplayName(),
                    title, body, subject:sEl?.value||'other',
                    upvotes:[], upvoteCount:0, replyCount:0, solved:false,
                    createdAt:serverTimestamp()
                });
                if(tEl) tEl.value='';
                if(bEl) bEl.value='';
                // Close new post panel
                ...
                if(errEl) errEl.textContent='Post failed — check your connection.';
                console.error('[p14] post error:',e);
            }
...
function _replaceForumSubmitReply() {
    function _try() {
        ...
        window.forumSubmitReply = async function() {
            if (!_db||!_uid) { _toast('Not logged in'); return; }
            // Get active post id — stored by forumOpenPost patch below
            const postId = window._p14_activePost;
            if (!postId) return;
            const bEl=document.getElementById('forum-reply-input');
            ...
            try {
                await addDoc(collection(_db,'forum_posts',postId,'replies'),{
                    uid:_uid, displayName:_getDisplayName(),
                    body, isAnswer:false, createdAt:serverTimestamp()
                });
                updateDoc(doc(_db,'forum_posts',postId),{replyCount:increment(1)}).catch(()=>{});
                if(bEl) bEl.value='';
                _toast('Reply posted ✓');
            } catch(e) {
                if(errEl) errEl.textContent='Reply failed — check connection.';
                console.error('[p14] reply error:',e);
            }
...
// Track active post so forumSubmitReply can use it
...
// Replace _srSubmit (fix_subthreads sub-replies) using our own addDoc
function _replaceSrSubmit() {
    function _try() {
        ...
        window._srSubmit = async function(postId, parentReplyId) {
            ...
            const ta  = document.getElementById('srta-'+parentReplyId);
            const btn = document.getElementById('srbtn-'+parentReplyId);
            if (!ta) return;
            ...
            try {
                await addDoc(collection(_db,'forum_posts',postId,'replies'),{
                    uid:_uid, displayName:_getDisplayName(),
                    body, parentReplyId, isAnswer:false, createdAt:serverTimestamp()
                });
                updateDoc(doc(_db,'forum_posts',postId),{replyCount:increment(1)}).catch(()=>{});
                ta.value='';
                // Close this inline box and all others
                document.querySelectorAll('[id^="srbox-"]').forEach(b=>b.style.display='none');
            } catch(e) {
                console.error('[p14] srSubmit error:',e);
...
// "Posting as" hint above new post form
function _forumNameHint() {
    function _try() {
        ...
        h.innerHTML=`<i class="fa-solid fa-user" style="color:var(--accent);font-size:.7rem;"></i>
            Posting as <strong id="p14-fhint-name" style="color:var(--text-main);margin-left:2px;">${_getDisplayName()}</strong>`;
...
/* ================================================================
   6. STUDY HABITS — local dates, week-start setting
   ================================================================ */
...
function _renderHabits(el) {
    ...
    const ws=_lsG('p9_week_start','mon');
    const sdow=ws==='sun'?0:1;
...
function _upgradeHabits() {
    // Force CSS injection to prevent the glitch
    if (!document.getElementById('p14-habits-style')) {
        const style = document.createElement('style');
        style.id = 'p14-habits-style';
        style.innerHTML = `
            .p14-wrow { background: rgba(255,255,255,0.05) !important; backdrop-filter: blur(10px); }
            .p14-wtog { width: 34px; height: 18px; border-radius: 20px; position: relative; }
            .p14-wtog.on { background: var(--accent) !important; }
        `;
...
/* ================================================================
   7. RE-RENDERS — name change, tab switch
   ================================================================ */
function _patchRerenders() {
    // Name change → greeting + forum hint
...
/* ================================================================
   8. SETTINGS → FIREBASE (personalise keys)
   ================================================================ */
let _syncT=null;
...
function _restorePersonalise(){function _t(){if(!window.DB?.get){setTimeout(_t,800);return;}const b=window.DB.get('os_personalise',null);if(!b)return;let n=0;Object.entries(b).forEach(([k,v])=>{if(localStorage.getItem(k)===null){try{localStorage.setItem(k,JSON.stringify(v));n++;}catch{}}});if(n)console.log('[p14] Restored',n,'personalise keys');}_setTimeout(_t,1800);}
...
(function init() {
    const go = () => {
        ...
        // Migrate old habit log to Firebase
        ...
        console.log('[patches14 v4] loaded');
    };
